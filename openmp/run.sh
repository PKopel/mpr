...
echo "size,n_threads,n_buckets,fill_time,split_time,merge_b_time,sort_time,merge_a_time,time_total,sorted"> results_2.csv

for i in {1..10}; do
    for s in 1000000 5000000; do
        for _ in {1..10}; do
            echo -n "$s," >> results_2.csv
            make run-$1 ARGS="$i 50 $s" >> results_2.csv
        done
    done
done
